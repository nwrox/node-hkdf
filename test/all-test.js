const vows = require("vows"),
    	 ...
    	 HKDF = require("../libs/hkdfbuffergenerator"),
	 suite = vows.describe('all');
...
suite.addBatch({
  "Test Case 1 - Basic test case with SHA-256" : {
	...
	let gen = new HKDF('sha256', ikm, info, salt, 42);

	return { OKM: gen.toString('hex'), PRK: HKDF.getPRK().toString('hex') };
    },
    "OKM and PRK": function(err, out) {
      assert.equal(
		out.OKM,
...
suite.addBatch({
  "Test Case 2 - Test with SHA-256 and longer inputs/outputs" : {
	...
	let gen = new HKDF('sha256', ikm, info, salt, 82);
...
suite.addBatch({
  "Test Case 4 - Basic test case with SHA-1" : {
	...
	let gen = new HKDF('sha1', ikm, info, salt, 42);
...
suite.addBatch({
  "Test Case 5 - Test with SHA-1 and longer inputs/outputs" : {
	...
	let gen = new HKDF('sha1', ikm, info, salt, 82);
